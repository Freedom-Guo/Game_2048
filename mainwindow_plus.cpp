...
void MainWindow::move_up(scores *p)
{
    ...
    for(j=0; j<4; j++)
    {
        for(i=0; i<3; i++)
        {
            ...
            {
                if(p->arg[i+2][j]!=0 && i+2<=3)
                {
                    p->arg[i][j] = p->arg[i+2][j];
                    p->arg[i+2][j] = 0;
                }
                else if(p->arg[i+2][j]==0&& i+2<=3)
                {
                    if(p->arg[i+3][j]!=0 && i+3<=3)
                    {
...
void MainWindow::move_down(scores *p)
{
    ...
    for(j=0; j<4; j++)
    {
        for(i=3; i>0; i--)
        {
            ...
            {
                if(p->arg[i-2][j]!=0 && i-2>=0)
                {
                    p->arg[i][j] = p->arg[i-2][j];
                    p->arg[i-2][j] = 0;
                }
                else if(p->arg[i-2][j]==0&& i-2>=0)
                {
                    if(p->arg[i-3][j]!=0 && i-3>=0)
                    {
                        p->arg[i][j] = p->arg[i-3][j];
                        p->arg[i-3][j] = 0;
                    }
...
void MainWindow::move_left(scores *p)
{
    ...
    for(i=0; i<4; i++)
    {
        for(j=0; j<3; j++)
        {
            ...
            {
                if(p->arg[i][j+2]!=0 && j+2<=3)
                {
                    p->arg[i][j] = p->arg[i][j+2];
                    p->arg[i][j+2] = 0;
                }
                else if(p->arg[i][j+2]==0&& j+2<=3)
                {
                    if(p->arg[i][j+3]!=0 && j+3<=3)
                    {
...
void MainWindow::move_right(scores *p)
{
    ...
    for(i=0; i<4; i++)
    {
        for(j=3; j>0; j--)
        {
            ...
            {
                if(p->arg[i][j-2]!=0 && j-2>=0)
                {
                    p->arg[i][j] = p->arg[i][j-2];
                    p->arg[i][j-2] = 0;
                }
                else if(p->arg[i][j-2]==0&& j-2>=0)
                {
                    if(p->arg[i][j-3]!=0 && j-3>=0)
                    {
                        p->arg[i][j] = p->arg[i][j-3];
                        p->arg[i][j-3] = 0;
                    }
